defmodule Souq.AccountsTest do
  use Souq.DataCase
  ...
  describe "tests" do
    ...
    test "update_test/2 with valid data updates the test" do
      test = test_fixture()
      assert {:ok, test} = Accounts.update_test(test, @update_attrs)
      assert %Test{} = test
    end
  ...
  describe "tests" do
    ...
    test "create_test/1 with valid data creates a test" do
      assert {:ok, %Test{} = test} = Accounts.create_test(@valid_attrs)
      assert test.username == "some username"
    end
    ...
    test "update_test/2 with valid data updates the test" do
      ...
      assert {:ok, test} = Accounts.update_test(test, @update_attrs)
      assert %Test{} = test
      assert test.username == "some updated username"
    end
